...
const AdminDashboard = {
  ...
  async init() {
    ...
    // Set admin name in welcome header and sidebar
  ...
  showEditDriveModal(courseId, courseName) {
    ...
    Modal.show('edit-drive', `
      <form onsubmit="AdminDashboard.handleUpdateDrive(event, '${courseId}')">
    ...
    list.innerHTML = paginated.map(s => `
      <tr>
        ...
        <td style="display:flex; gap:6px;">
          ...
          <button onclick="AdminDashboard.goToMonitor('${s._id}')" class="btn btn-outline btn-sm">Monitor</button>
          <button onclick="AdminDashboard.goToAnalytics('${s._id}')" class="btn btn-outline btn-sm">Results</button>
          <button onclick="AdminDashboard.deleteSession('${s._id}')" class="btn btn-outline btn-sm" style="color:var(--danger); border-color:rgba(239, 68, 68, 0.2);"><i class="fas fa-trash"></i></button>
        </td>
      </tr>
    `).join('');

    if (paginationContainer) {
      this.renderSquarePagination('recent-sessions-pagination', p, totalPages, 'AdminDashboard.renderSessions(null, ');
    }
  },
  ...
  renderResults(results, page = null) {
    ...
    if (paginationContainer) {
      this.renderSquarePagination('recent-results-pagination', p, totalPages, 'AdminDashboard.renderResults(null, ');
    }
  },
    ...
    try {
      ...
      if (!this.currentBoardFolder) {
        ...
        container.innerHTML = Object.keys(groupedBanks).sort().map(board => `
          <div class="mcq-bank-card animate-slide-up" style="cursor: pointer; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 40px 20px; text-align: center; border: 2px dashed #cbd5e1; background: #f8fafc;" onclick="AdminDashboard.openBoardFolder('${board}')">
        ...
        let html = `
          <div style="grid-column: 1/-1; margin-bottom: 24px; display: flex; align-items: center; justify-content: space-between; border-bottom: 2px solid #f1f5f9; padding-bottom: 16px;">
            <div style="display: flex; align-items: center; gap: 16px;">
              <button class="btn btn-secondary btn-sm" style="border-radius: 12px; padding: 8px 16px; display: flex; align-items: center; gap: 8px;" onclick="AdminDashboard.closeBoardFolder()">
        ...
        html += banks.map(bank => `
          <div class="mcq-bank-card animate-slide-up" onclick="AdminDashboard.previewMCQBank('${bank._id}')">
            ...
            <div class="mcq-bank-actions">
              ...
              <button onclick="event.stopPropagation(); AdminDashboard.editMCQBank('${bank._id}')" class="btn btn-secondary btn-sm"><i class="fas fa-edit"></i></button>
              <button onclick="event.stopPropagation(); AdminDashboard.deleteMCQBank('${bank._id}')" class="btn btn-secondary btn-sm" style="color: var(--danger);"><i class="fas fa-trash"></i></button>
            </div>
          </div>
        `).join('');
          ...
          <div class="glass-card" id="report-card-${session._id}" style="margin-bottom: 48px; padding: 40px;">
            <div class="session-header-flex" style="margin-bottom: 40px;">
              ...
              <div class="session-actions-flex">
                <button class="btn btn-outline" style="font-size: 12px; padding: 8px 16px; border-color: #10b981; color: #10b981;" onclick="AdminDashboard.exportToExcel('${session._id}')">
                  <i class="fas fa-file-excel"></i> Export Excel
                </button>
                <button class="btn btn-outline" style="font-size: 12px; padding: 8px 16px;" onclick="AdminDashboard.exportToPDF('${session._id}')">
      ...
      ${totalPages > 1 ? `
        <div class="pagination-wrapper">
          <div class="pagination-pill">
            <button class="page-btn text-btn" ${page === 1 ? 'disabled' : ''} onclick="AdminDashboard.renderAnalyticsExamsPage(${page - 1})">&larr; Prev</button>
            ${(() => {
              let html = '';
              ...
              if (!showEllipsis) {
                ...
                for(let i=1; i<=totalPages; i++) {
                  html += `<button class="page-btn num-btn ${i === page ? 'active' : ''}" onclick="AdminDashboard.renderAnalyticsExamsPage(${i})">${i}</button>`;
                }
              } else {
                ...
                if (page <= 3) {
                  ...
                  html += `<button class="page-btn num-btn" onclick="AdminDashboard.renderAnalyticsExamsPage(${totalPages})">${totalPages}</button>`;
                } else if (page >= totalPages - 2) {
                  html += `<button class="page-btn num-btn" onclick="AdminDashboard.renderAnalyticsExamsPage(1)">1</button>`;
                  html += `<span class="page-btn num-btn ellipsis">...</span>`;
                  ...
                  html += `<button class="page-btn num-btn" onclick="AdminDashboard.renderAnalyticsExamsPage(1)">1</button>`;
                  html += `<span class="page-btn num-btn ellipsis">...</span>`;
                  html += `<button class="page-btn num-btn active" onclick="AdminDashboard.renderAnalyticsExamsPage(${page})">${page}</button>`;
                  html += `<span class="page-btn num-btn ellipsis">...</span>`;
                  html += `<button class="page-btn num-btn" onclick="AdminDashboard.renderAnalyticsExamsPage(${totalPages})">${totalPages}</button>`;
                }
              }
            ...
            <button class="page-btn text-btn" ${page === totalPages ? 'disabled' : ''} onclick="AdminDashboard.renderAnalyticsExamsPage(${page + 1})">Next &rarr;</button>
          </div>
        </div>
      ` : ''}
    `;
  ...
  editMCQBank(bankId) {
    ...
    Modal.show('edit-bank', `
      <form id="edit-bank-form" onsubmit="AdminDashboard.handleEditMCQBank(event, '${bank._id}')">
        <div class="form-group">
      ...
      body.innerHTML = filtered.map(m => `
        <tr>
          ...
          <td style="display:flex; gap:8px;">
            <button onclick="AdminDashboard.viewMaterial('${m._id}', '${m.url}')" class="btn btn-outline" style="padding:4px 8px; color:var(--primary);">View</button>
            <button onclick="AdminDashboard.deleteMaterial('${m._id}')" class="btn btn-outline" style="color:#ef4444; padding:4px 8px;">Delete</button>
          </td>
        </tr>
      `).join('');
    } catch (err) {
  ...
  showUploadMaterial() {
    ...
    Modal.show('upload-material', `
      <div style="position: relative; padding: 10px;">
        ...
        <form id="upload-material-form" onsubmit="AdminDashboard.handleUploadMaterial(event)" style="display: flex; flex-direction: column; gap: 24px;">
          <!-- Primary Info Grid -->
          ...
          <div style="display: grid; grid-template-columns: 1fr 1.5fr; gap: 20px;">
            <div class="form-group">
              ...
              <select name="type" class="form-control" style="width: 100%; height: 44px;" onchange="AdminDashboard.toggleMaterialInput(this.value)">
    ...
    try {
      ...
      body.innerHTML = filtered.map(s => `
        <tr>
          ...
          <td>
            <button onclick="AdminDashboard.showEditStudentModal('${s._id}')" class="btn btn-outline" style="padding:4px 8px; font-size:11px;">Edit</button>
            <button onclick="AdminDashboard.handleDeleteStudent('${s._id}')" class="btn btn-outline" style="padding:4px 8px; font-size:11px; color:var(--danger); border-color:rgba(239, 68, 68, 0.2);">Delete</button>
          </td>
        </tr>
      `).join('');
    } catch (err) {
  ...
  async showAddStudentModal() {
    ...
    Modal.show('add-student', `
      <div style="position: relative; padding: 10px;">
        ...
        <form onsubmit="AdminDashboard.handleAddStudent(event)" style="display: flex; flex-direction: column; gap: 24px;">
          <!-- Primary Info Grid -->
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
            <div class="form-group">
  ...
  async showEditStudentModal(id) {
    try {
      ...
      Modal.show('edit-student', `
        <form onsubmit="AdminDashboard.handleEditStudent(event, '${id}')">
          <div class="form-group">
  ...
  async showManualAttendance() {
    ...
    try {
      ...
      Modal.show('manual-attendance', `
        <form onsubmit="AdminDashboard.handleManualAttendance(event, '${sessionId}')">
          ...
          <div style="display: flex; gap: 10px; margin-bottom: 16px;">
            <button type="button" onclick="AdminDashboard.markAllAttendance('present')"
            ...
            <button type="button" onclick="AdminDashboard.markAllAttendance('absent')"
    ...
    try {
      ...
      body.innerHTML = students.map(att => `
        <tr>
          ...
          <td style="display:flex; gap:8px;">
            <button onclick="AdminDashboard.markAttendanceStatus('${sessionId}', '${att.studentId?._id}', 'present')" class="btn btn-outline" style="padding:4px 8px; font-size:11px;">Present</button>
            <button onclick="AdminDashboard.markAttendanceStatus('${sessionId}', '${att.studentId?._id}', 'absent')" class="btn btn-outline" style="padding:4px 8px; font-size:11px;">Absent</button>
            <button onclick="AdminDashboard.deleteAttendanceRecord('${att._id}', '${sessionId}')" class="btn btn-outline" style="padding:4px 8px; font-size:11px; color:#ef4444;"><i class="fas fa-trash"></i></button>
          </td>
        </tr>
      `).join('');
    } catch (err) {
  ...
  loadForum() {
    const container = document.getElementById('admin-forum-container');
    container.innerHTML = `
      <header class="flex-between" style="margin-bottom: 32px;">
        <div>
          <h2 class="h2">Forum Management</h2>
          <p class="p-dim">Admins can monitor discussions and guide student queries.</p>
        </div>
      </header>
  ...
  async showBroadcastModal() {
    ...
    Modal.show('announcement', `
      ...
      <button onclick="AdminDashboard.sendAnnouncement()" class="btn btn-primary" style="width:100%;">Broadcast to Students</button>
  ...
  async sendAnnouncement() {
    ...
    try {
      ...
      if (window.AdminSocket && AdminSocket.socket) {
        AdminSocket.socket.emit('broadcast-announcement', { title, content, courseId });
      }
      // Save to DB
