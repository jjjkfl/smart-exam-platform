...
const parseAdda247Format = (blocks) => {
  ...
  let currentQ = null, lastOptLabel = null;
  ...
  const save = () => {
    ...
    currentQ = null; lastOptLabel = null; parserState = 'idle';
  };
  ...
  for (const { text, imgSrc } of blocks) {
    ...
    if (isNewQ) {
      ...
      parserState = 'in_answer';
      const clean = text.replace(/^(?:QUESTION\s+(?:NO\.?\s*)?\d+[\.\:\s]*|Q\.?\s*\d+[\.\:\)\-]\s*|\d+[\.\)\:\-]\s*)/i, '').trim();
      if (clean) { currentQ.questionText = clean; parserState = 'in_question'; }
      continue;
    }
    ...
    const expM = text.match(/^(?:Explanation|Exp|Rationale|Solution)\s*[\:\-\s]*(.*)/i);
    if (expM && currentQ) {
      currentQ.explanation = (currentQ.explanation || '') + ' ' + expM[1].trim();
      parserState = 'in_explanation';
      continue;
    }
    ...
    if (optM) {
      ...
      parserState = 'in_options'; continue;
    }

    if (parserState === 'in_options' && lastOptLabel) {
...
const parseGenericFormat = (blocks) => {
  ...
  for (let i = 0; i < expanded.length; i++) {
    ...
    if (qMatch || isUnnumbered) {
      ...
      pendingImage = null; lastOptLabel = null; parserState = 'in_question';
      continue;
    }
    ...
    // Explicit explanation line
    const expM = text.match(/^(?:Explanation|Exp|Rationale|Solution|Detail|Ans Detail|Ans Explanation)\s*[\:\-\s]*(.*)/i);
    if (expM && currentQ) {
      currentQ.explanation = (currentQ.explanation || '') + ' ' + expM[1].trim();
      parserState = 'in_explanation';
      continue;
    ...
    // Continuation of question text or explanation based on state
    if (parserState === 'in_explanation' && currentQ) {
      currentQ.explanation = (currentQ.explanation || '') + ' ' + text.trim();
    } else if (parserState === 'in_question' && currentQ && currentQ.options.length === 0) {
